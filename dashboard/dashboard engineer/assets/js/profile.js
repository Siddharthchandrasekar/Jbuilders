      ...
      snapshot.forEach((childSnapshot) => {
        ...
        if (userData.userId === userId) {
          ...
          if (userData.image) {
            ...
            // Set alternate image if no image in the database
            document.getElementById("preview").src = "https://tse3.mm.bing.net/th?id=OIP.Cl56H6WgxJ8npVqyhefTdQHaHa&pid=Api&P=0&h=180"; // Replace with your alternate image URL
          }
          console.log("done")
        }
      });
...
document.getElementById("update").addEventListener("click", function () {
  const userId = "TO2IS2zUwAeEAWsdbTY5bOcywC92"; // Replace with the actual user ID
    ...
    if (snapshot.exists()) {
      ...
      snapshot.forEach((childSnapshot) => {
        ...
        if (userData.userId === userId) {
          ...
          const image = document.getElementById("preview").src; // Get the image source
